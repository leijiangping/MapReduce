...
/**
 * Created by xiaohei on 16/2/22. 驱动程序设置类,在各个mr的驱动程序中调用此类的InitJob方法即可初始化Job
 */
public class BaseDriver {
	 ...
	 *
	 * @param jobs
	 *            一个或多个保存着job信息的JobInitModel对象 TODO:可根据需要进行修改,如添加自定义的分组,排序,规约类的设置
	 */
...
/*		JobControl jc = new JobControl("My control job");
		...
		/*	ControlledJob cjob = new ControlledJob(job.getConfiguration());
			...
			//
			if(beforeJob!=null) {
				cjob.addDependingJob(beforeJob);
			}
			// 将受控作业添加到控制器中
			jc.addJob(cjob);*/
		}
